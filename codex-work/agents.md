...
- The `Experience`/`Academic` page now renders an `Academic Background` module immediately above `Current Research Interests`. The module is local to `generated-site/src/pages/ExperiencePage.jsx` and lists GPA, language ability, and undergraduate courses; styling lives in `generated-site/src/App.css` under `.academic-background-*`.
- Latest verification commands run for the Academic Background module: `npm run lint` and `npm run build`. Visual screenshot QA was skipped because Playwright is not installed in `generated-site/node_modules` and no Chrome/Edge executable was available on PATH.
- Follow-up Academic Background copy change: all user-visible text in that module is now English, including the description `The following is some of my basic personal academic information.`, GPA equivalence, language ability, and `Undergraduate Courses Taken`. Latest verification commands: `npm run lint` and `npm run build`.
- Follow-up Academic Background heading resize: `.academic-background-heading h2` now uses the same heading scale as the Research Focus module instead of the previous hero-scale type. Latest verification commands: `npm run lint` and `npm run build`.
- Follow-up Academic Background localization: `ExperiencePage.jsx` now uses `academicBackgroundCopy` plus localized item labels/values so the module switches between English and Chinese with the page language. Course names remain the provided official English titles while the section heading is localized. Latest verification commands: `npm run lint` and `npm run build`.
...
- The Dance video hero now overlays an accessible `Welcome to My Dance Space` H1. The title is rendered as word groups with individually animated letter spans, styled in `generated-site/src/App.css` with `.dance-hero-title-*` classes and pure CSS keyframes so the words enter from the left and keep a light bounce at center.
- Latest verification commands run for the animated dance hero title: `npm run lint` and `npm run build`.
- Follow-up title animation change: `.dance-letter-hop-in` now starts each letter at `translateX(calc(-100vw - 10rem))`, so the letters visibly jump in from outside the left edge of the screen rather than only sliding the title container. Latest verification commands: `npm run lint` and `npm run build`.
- Current follow-up behavior: the Dance hero title now stays arranged as a single row and the full row uses `.dance-title-row-hop-in` to slowly hop from offscreen left into the centered final position. Individual letters no longer animate independently on entry; they are only spans for layout. Latest verification commands: `npm run lint` and `npm run build`.
- Latest title behavior: `DancePage.jsx` renders the title as two fixed lines, `Welcome to My` and `Dance Space`. `.dance-hero-title` now uses `.dance-title-walk-in` for a continuous smooth left-to-center entrance, while `.dance-hero-title-line` adds a subtle alternating vertical bob for a walking feel without stepped horizontal movement. Latest verification commands: `npm run lint` and `npm run build`.
- The Dance hero now includes a small `DANCE VIDEO` eyebrow above the two-line title, matching the Academic hero's label placement. The eyebrow and title are wrapped in `.dance-hero-title-group`, so the full group shares the smooth left-to-center entrance and settled breathing animation. Latest verification commands: `npm run lint` and `npm run build`.
